...
INT16U SerialCommuModbus::InterruptControlCoilPackagePrepare(QUEUE_MSG *pMsg, INT8U *buffer)
{
    ...
    if(pDeviceDataBase->DeviceValue[tLogicAddr].GroupIndex < MAX_CHANNEL_NUM)
        tRegAddr = pDeviceDataBase->SysCfgData.ProtoCfgData[tTypeIndex][tVerIndex].SwitchControlSection.Point[pDeviceDataBase->DeviceValue[tLogicAddr].GroupIndex].addr;
