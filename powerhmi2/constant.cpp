...
void Constant::EnterProcess(INT32S LogicAddr)
{
    qDebug()<<"constant EnterProcess";
    DisplayConstantInfo(LogicAddr,GROUP_INDEX_ONE);
//    pDeviceDataBase->DeviceValue[LogicAddr].ConstantReadStep = READ_CONSTANT_UNDO;
    mEnterFlag = 1;
    mConstantG1ReadStep = READ_CONSTANT_UNDO;
    mConstantG2ReadStep = READ_CONSTANT_UNDO;
    if(pDeviceDataBase->DeviceValue[LogicAddr].commu.status == COMM_STATUS_ON)
        ReadConstant(LogicAddr,GROUP_INDEX_ONE);
}
...
void Constant::DisplayConstantInfo(INT32S LogicAddr, INT32S GroupIndex)
{
    ...
    {
        ui->label_INFO->show();
        ui->label_INFO->setText(tr("该设备设置为单次写一个定值"));
    }
    else if(mWriteConstantFunCode == WriteMoreHoldReg)
    {
        ui->label_INFO->hide();
    }
    else
    {
        qDebug()<<"Wrong wrote constant code";
    }
    if(mMaxGroupNum == 2)
    {
...
void Constant::key_f1_press()
{
    ...
    case FLAG_READ:
        ...
        break;
    case FLAG_WRITE:
        ui->read->setStyleSheet("background-color: rgb(255, 255, 255);");
        ui->write->setStyleSheet("background-color: rgb(0, 255, 0);");
        break;
    }
}
...
void Constant::key_f3_press()
{
    if(mMaxGroupNum == 2)
    {
        ...
        {
            mGroupIndex = 0;
            ui->label_Group1->setStyleSheet("background-color: rgb(255, 255, 255);");
            ui->label_Group2->setStyleSheet("background-color: rgb(0, 255, 0);");
        }
    }
}
...
//void Constant::handle_constant_display1(bool check)
//{
//    if(check)
//    {
//       // if(group == 0)
//        //    group_num = dev_info.num;
//        //else
//        //    group_num = MAX_VAL_NUM;
//        ui->tableWidget->setRowCount(group_num);
//        for(int i = 0;i < group_num;i++)
//        {
//       //     ui->tableWidget->setItem(i,0,new QTableWidgetItem(dev_info.info[i].name));
//            ui->tableWidget->setItem(i,1,new QTableWidgetItem("0"));
//        //    ui->tableWidget->setItem(i,2,new QTableWidgetItem(dev_info.info[i].comment));
//        }
//        select_zone = 1;
//    }
//}
//void Constant::handle_constant_display2(bool check)
//{
//    if(check)
//    {
//       // group_num = dev_info.num % MAX_VAL_NUM;
//        ui->tableWidget->setRowCount(group_num);
//        for(int i = 0;i < group_num;i++)
//        {
//           // ui->tableWidget->setItem(i,0,new QTableWidgetItem(dev_info.info[i + MAX_VAL_NUM].name));
//            ui->tableWidget->setItem(i,1,new QTableWidgetItem("0"));
//           // ui->tableWidget->setItem(i,2,new QTableWidgetItem(dev_info.info[i + MAX_VAL_NUM].comment));
//        }
//        select_zone = 2;
//    }
//}
