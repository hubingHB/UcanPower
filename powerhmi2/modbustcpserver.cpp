...
INT32S modbustcpserver::handleReadHoldReg(INT16U reg_start, INT16U reg_num, INT32S recv_size)//0x03 read
{
    ...
    if((reg_start >= HoldReg_06_START)&&((reg_start + reg_num - 1) <= HoldReg_06_END))
    {
        len = handleReadHoldRegProcessing(reg_start,reg_num);

    }
    else if((reg_start >= HoldReg_06_SINGLE_START)&&((reg_start + reg_num - 1) <= HoldReg_06_SINGLE_END))
    {
        len = handleReadHoldRegSingleProcessing(reg_start,reg_num);
    }
    else if((reg_start >= HoldReg_06_CONTROL_START)&&((reg_start + reg_num - 1) <= HoldReg_06_CONTROL_END))
    {
        serverPointTable.HoldRegControlTable[0] = 0;
        serverPointTable.HoldRegControlTable[1] = pDeviceDataBase->StationRLMode;//pCurrentStatus->mode;
...
INT32S modbustcpserver::handleWriteHoldReg(INT16U reg_start, INT16U reg_num, INT32S recv_size)//0x06
{
    ...
    if(reg_start == HoldReg_06_CONTROL_START) // remote/local
    {
        ...
        serverPointTable.HoldRegControlTable[0] = 0;
        serverPointTable.HoldRegControlTable[1] = pDeviceDataBase->StationRLMode;
        emit remote_switch_local_remote_signal(pDeviceDataBase->StationRLMode);
    }
    else if((reg_start > HoldReg_06_CONTROL_START)&&(reg_start <= HoldReg_06_CONTROL_END))
    {
        logicIndex = reg_start - HoldReg_06_CONTROL_START - 1;
        ...
        if((tCMD == CMD_CONSTANT_READ)&&(tChannelOrGroupId < 2))
        {
            ...
            serverPointTable.HoldRegControlTable[(logicIndex+1)*2] = (tStatus>>8)&0xFF;
            serverPointTable.HoldRegControlTable[(logicIndex+1)*2 + 1] = tStatus&0xFF;
    ...
    else if((reg_start >= HoldReg_06_START)&&(reg_start <= HoldReg_06_END)) //将写的定值暂存在这边,一定要先读定值才能写
    {
        pointIndex = reg_start - HoldReg_06_START;
        serverPointTable.ConstantData[pointIndex] = reg_num;
    }
    else if((reg_start >= HoldReg_06_SINGLE_START)&&(reg_start <= HoldReg_06_SINGLE_END)) //将写的定值暂存在这边,一定要先读定值才能写
    {
        ...
        if((tPointTableIndex >= 0)&&(tPointTableIndex < (100-1)))
        {
            ...
            if(FrameType == FRAME_CONSTANT_WRITE)
            {
                 if(pDeviceDataBase->DeviceValue[LogicAddr].Result == RESULT_SUCCESS)
                 {
                    ...
                    serverPointTable.HoldRegControlTable[(tPointTableIndex + 1)*2]  = (tStatusCMD>>8)&0xFF;
                    serverPointTable.HoldRegControlTable[(tPointTableIndex + 1)*2+1] = tStatusCMD&0xFF;
                 }
                 else
                 {
